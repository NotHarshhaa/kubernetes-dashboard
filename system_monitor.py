...
metrics_history = {"cpu": [], "memory": [], "disk": [], "timestamp": []}
...
def get_system_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            mem = type(
                "MemoryInfo", (), {"total": 0, "available": 0, "used": 0, "percent": 0}
            )()
        ...
        cpu_percent = (
            max(0, min(100, float(cpu_percent))) if cpu_percent is not None else 0.0
        )
        mem_percent = (
            max(0, min(100, float(mem.percent)))
            if hasattr(mem, "percent") and mem.percent is not None
            else 0.0
        )
        disk_percent = (
            max(0, min(100, float(disk.percent)))
        ...
        data = {
            ...
            "cpu_details": {
                ...
                "frequency": {
                    "current": (
                        round(cpu_freq.current, 2)
                        if cpu_freq
                        and hasattr(cpu_freq, "current")
                        and cpu_freq.current
                        else None
                    ),
                    "min": (
                        round(cpu_freq.min, 2)
                        if cpu_freq and hasattr(cpu_freq, "min") and cpu_freq.min
                        else None
                    ),
                    "max": (
                        round(cpu_freq.max, 2)
                        if cpu_freq and hasattr(cpu_freq, "max") and cpu_freq.max
                        else None
                    ),
                },
                "per_core": (
                    [round(core, 2) for core in cpu_per_core] if cpu_per_core else []
                ),
            },
            "memory_usage": {
        ...
        with _metrics_lock:
            ...
            if len(metrics_history["cpu"]) > METRICS_HISTORY_LIMIT:
                ...
                metrics_history["memory"] = metrics_history["memory"][
                    -METRICS_HISTORY_LIMIT:
                ]
                metrics_history["disk"] = metrics_history["disk"][
                    -METRICS_HISTORY_LIMIT:
                ]
                metrics_history["timestamp"] = metrics_history["timestamp"][
                    -METRICS_HISTORY_LIMIT:
                ]
        ...
        logger.debug(
            f"Successfully collected system metrics: CPU={cpu_percent}%, MEM={mem_percent}%, DISK={disk_percent}%"
        )
        return data

    except Exception as e:
        logger.error(
            f"❌ Critical error getting system metrics: {str(e)}", exc_info=True
        )
